...
async function testBookingWithServiceRole() {
  console.log('Testing booking system with service role client...');
  ...
  try {
    ...
    if (guidesError) {
      ...
      if (guides && guides.length > 0) {
        // Try to use id first, fall back to user_id
        validGuideId = guides[0].id || guides[0].user_id;
  ...
  // 2. Try to insert a test booking with a valid guide ID using admin client
  try {
    ...
    if (insertError) {
      ...
      console.log('Test booking inserted successfully with admin client:', insertResult);
      console.log('Booking ID:', bookingId);
    }
    
    // Check if the booking was actually inserted using admin client
    ...
    // Also try to verify with regular client to check if RLS is blocking
    ...
    if (regularVerifyError) {
      ...
      console.log('Regular client verification result:', regularVerifyData);
